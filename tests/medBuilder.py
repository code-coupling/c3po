...
def makeMesh3DCart(sizeX, sizeY, sizeZ, nMeshX, nMeshY, nMeshZ, offset=None):
    coord = offset[0] if offset is not None else 0
    coordsX = []
    for i in range(nMeshX + 1):
        coordsX.append(coord)
        coord += sizeX / nMeshX
    coord = offset[1] if offset is not None else 0
    coordsY = []
    for i in range(nMeshY + 1):
        coordsY.append(coord)
        coord += sizeY / nMeshY
    coord = offset[2] if offset is not None else 0
    coordsZ = []
...
def makeField3DCart(sizeX, sizeY, sizeZ, nMeshX, nMeshY, nMeshZ, offset=None):
    f = mc.MEDCouplingFieldDouble.New(mc.ON_CELLS)
    f.setMesh(makeMesh3DCart(sizeX, sizeY, sizeZ, nMeshX, nMeshY, nMeshZ, offset))
