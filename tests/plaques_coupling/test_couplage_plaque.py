...
taille = 1
nbrNoeuds = 20
# Temperature de reference pour nbrNoeuds = 20 et calcule avec un FixedPointCoupler et une precision de 1E-15
...
def couplage_plaque(coupler_type='FixedPoint'):
    ...
    Ph1toPhy2 = c3po.LocalExchanger(c3po.DirectMatching(), [], [], [(myPhysics1, str(i)) for i in range(nbrNoeuds)], [(myPhysics2, str(i)) for i in range(nbrNoeuds)])
    Ph2toData = c3po.LocalExchanger(c3po.DirectMatching(), [], [], [(myPhysics2, str(i)) for i in range(nbrNoeuds)], [(DataCoupler, str(i)) for i in range(nbrNoeuds)])
    DatatoPh1 = c3po.LocalExchanger(c3po.DirectMatching(), [], [], [(DataCoupler, str(i)) for i in range(nbrNoeuds)], [(myPhysics1, str(i)) for i in range(nbrNoeuds)])
    ...
    if coupler_type == "FixedPoint":
        ...
        exch_Residual1 = c3po.LocalExchanger(c3po.DirectMatching(), [], [], valuesToGet=[(myPhysics1, 'PRECISION_ATTEINTE')], valuesToSet=[(DataCouplerResiduals, 'Residual1')])
        exch_Residual2 = c3po.LocalExchanger(c3po.DirectMatching(), [], [], valuesToGet=[(myPhysics2, 'PRECISION_ATTEINTE')], valuesToSet=[(DataCouplerResiduals, 'Residual2')])

        # Pour que l'algorithme puisse setter Accuracy.
        myPhysics1RB = c3po.NameChanger(myPhysics1, nameMappingValue={"Accuracy": "PRECISION"})
        myPhysics2RB = c3po.NameChanger(myPhysics2, nameMappingValue={"Accuracy": "PRECISION"})

        if coupler_type == 'AdaptiveResidualBalance':
            couplerClass = c3po.AdaptiveResidualBalanceCoupler
        else:
            couplerClass = c3po.DynamicResidualBalanceCoupler
        CouplerResidualBalance = couplerClass(
                {"Solver1": myPhysics1RB, "Solver2": myPhysics2RB},
                {"1to2": Ph1toPhy2,
        ...
        # Calculs des etats initiaux perturbes
        myPhysics1.iterate()  # Implique le calcul d'une iteration/d'un pas de temps
        Ph1toPhy2.exchange()
        myPhysics2.iterate()  # Implique le calcul d'une iteration/d'un pas de temps
        Ph2toData.exchange()
        DatatoPh1.exchange()
        ...
        myFixedPointCoupler.term()

    # Recuperation de la nappe de temperature des deux plaques
    ...
    # Verification de la nappe de temperature obtenue
    for i in range(nbrNoeuds):
        for j in range(nbrNoeuds):
            assert pytest.approx(temperature[i][j], abs=1.E-6) == temperature_ref[i][j]
