...
def test_grid():
    ...
    fuelHexaGrid = c3po.multi1D.MultiLevelGrid(fuelGrid, [toutepetitGrid]*12 + [emptyGrid] + [toutepetitGrid]*12)
    assemblyGrid = c3po.multi1D.CartesianGrid([5.5]*2, [5.5]*2)
    coreGrid = c3po.multi1D.CartesianGrid([11.5]*4, [11.5]*4)
    hexaassembly = c3po.multi1D.MultiLevelGrid(assemblyGrid, [fuelHexaGrid]*4)
    emptyAssembly = c3po.multi1D.MultiLevelGrid(emptyGrid, [])
    core = c3po.multi1D.MultiLevelGrid(coreGrid, [emptyAssembly, hexaassembly, hexaassembly, emptyAssembly] + [hexaassembly]*8 + [emptyAssembly, hexaassembly, hexaassembly, emptyAssembly])
    core.shift(1., 2.)
    ref = []
